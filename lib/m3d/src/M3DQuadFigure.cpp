...
Vector3D M3DQuadFigure::getCOG(bool all) const
{
    ...
    M3DFigure * tempFigure = this->clone();

    MethodOfMoments::figureMoments(tempFigure, volume, center, rotation);
...
// This function is computing derivatives of position on 2d s-rep
Vector3D M3DQuadFigure::computeDX(int row)
{
    Vector3D dx;
    if(row == 0)
    {
        dx = getPrimitivePtr(row+1, 0)->getX() - getPrimitivePtr(row, 0)->getX();
    }
    else if(row == numRows - 1)
    {
        dx = getPrimitivePtr(row, 0) ->getX() - getPrimitivePtr(row-1, 0)->getX();
    }
    else
    {
        dx = (getPrimitivePtr(row+1,0)->getX() - getPrimitivePtr(row-1,0)->getX()) / 2.0;
    }
    
    return dx;
}
// These functions use central differences unless on an edge, then use forward/backward where appropriate

Vector3D M3DQuadFigure::computeDXdu(int rowIndex, int colIndex)
{
	if (rowIndex < 0 || rowIndex > numRows - 1 || colIndex < 0 || colIndex > numColumns - 1)
	{
		std::cout << "Invalid row or column: row " << rowIndex << ", column " << colIndex << std::endl;
	}
	Vector3D dXdu;
	if (rowIndex == 0) // On left edge, use forward diff
		dXdu = getPrimitivePtr(rowIndex+1,colIndex)->getX() - getPrimitivePtr(rowIndex,colIndex)->getX();
	else if (rowIndex == numRows - 1) // On right edge, use backward diff
		dXdu = getPrimitivePtr(rowIndex,colIndex)->getX() - getPrimitivePtr(rowIndex-1,colIndex)->getX();
	else
		dXdu = (getPrimitivePtr(rowIndex+1,colIndex)->getX() - getPrimitivePtr(rowIndex-1,colIndex)->getX()) / 2.0;

	return dXdu;
}

Vector3D M3DQuadFigure::computeDXdv(int rowIndex, int colIndex)
{
	if (rowIndex < 0 || rowIndex > numRows - 1 || colIndex < 0 || colIndex > numColumns - 1)
	{
		std::cout << "Invalid row or column: row " << rowIndex << ", column " << colIndex << std::endl;
	...
	Vector3D dXdv;
	if (colIndex == 0) // On top edge, use forward diff
		dXdv = getPrimitivePtr(rowIndex,colIndex+1)->getX() - getPrimitivePtr(rowIndex,colIndex)->getX();
	else if (colIndex == numColumns - 1) // On bottom edge, use backward diff
		dXdv = getPrimitivePtr(rowIndex,colIndex)->getX() - getPrimitivePtr(rowIndex,colIndex-1)->getX();
	else
		dXdv = (getPrimitivePtr(rowIndex,colIndex+1)->getX() - getPrimitivePtr(rowIndex,colIndex-1)->getX()) / 2.0;

	return dXdv;
}

Vector3D M3DQuadFigure::computeDU0du(int rowIndex, int colIndex)
{
	if (rowIndex < 0 || rowIndex > numRows - 1 || colIndex < 0 || colIndex > numColumns - 1)
	{
		std::cout << "Invalid row or column: row " << rowIndex << ", column " << colIndex << std::endl;
	...
	Vector3D dU0du;
	if (rowIndex == 0) // On left edge, use forward diff
		dU0du = getPrimitivePtr(rowIndex+1,colIndex)->getU0() - getPrimitivePtr(rowIndex,colIndex)->getU0();
	else if (rowIndex == numRows - 1) // On right edge, use backward diff
		dU0du = getPrimitivePtr(rowIndex,colIndex)->getU0() - getPrimitivePtr(rowIndex-1,colIndex)->getU0();
	else // In the interior, use central diff
		dU0du = (getPrimitivePtr(rowIndex+1,colIndex)->getU0() - getPrimitivePtr(rowIndex-1,colIndex)->getU0()) / 2;

	return dU0du;
}
Vector3D M3DQuadFigure::computeDU1du(int rowIndex, int colIndex)
{
	if (rowIndex < 0 || rowIndex > numRows - 1 || colIndex < 0 || colIndex > numColumns - 1)
	{
		std::cout << "Invalid row or column: row " << rowIndex << ", column " << colIndex << std::endl;
	...
	Vector3D dU1du;
	if (rowIndex == 0) // On left edge, use forward diff
		dU1du = getPrimitivePtr(rowIndex+1,colIndex)->getU1() - getPrimitivePtr(rowIndex,colIndex)->getU1();
	else if (rowIndex == numRows - 1) // On right edge, use backward diff
		dU1du = getPrimitivePtr(rowIndex,colIndex)->getU1() - getPrimitivePtr(rowIndex-1,colIndex)->getU1();
	else // In the interior, use central diff
		dU1du = (getPrimitivePtr(rowIndex+1,colIndex)->getU1() - getPrimitivePtr(rowIndex-1,colIndex)->getU1()) / 2;

	return dU1du;
}
Vector3D M3DQuadFigure::computeDU0dv(int rowIndex, int colIndex)
{
	if (rowIndex < 0 || rowIndex > numRows - 1 || colIndex < 0 || colIndex > numColumns - 1)
	{
		std::cout << "Invalid row or column: row " << rowIndex << ", column " << colIndex << std::endl;
	...
	Vector3D dU0dv;
	if (colIndex == 0) // On top edge, use forward diff
		dU0dv = getPrimitivePtr(rowIndex,colIndex+1)->getU0() - getPrimitivePtr(rowIndex,colIndex)->getU0();
	else if (colIndex == numColumns - 1) // On bottom edge, use backward diff
		dU0dv = getPrimitivePtr(rowIndex,colIndex)->getU0() - getPrimitivePtr(rowIndex,colIndex-1)->getU0();
	else // In the interior, use central diff
		dU0dv = (getPrimitivePtr(rowIndex,colIndex+1)->getU0() - getPrimitivePtr(rowIndex,colIndex-1)->getU0()) / 2;

	return dU0dv;
}
Vector3D M3DQuadFigure::computeDU1dv(int rowIndex, int colIndex)
{
	if (rowIndex < 0 || rowIndex > numRows - 1 || colIndex < 0 || colIndex > numColumns - 1)
	{
		std::cout << "Invalid row or column: row " << rowIndex << ", column " << colIndex << std::endl;
	...
	Vector3D dU1dv;
	if (colIndex == 0) // On top edge, use forward diff
		dU1dv = getPrimitivePtr(rowIndex,colIndex+1)->getU1() - getPrimitivePtr(rowIndex,colIndex)->getU1();
	else if (colIndex == numColumns - 1) // On bottom edge, use backward diff
		dU1dv = getPrimitivePtr(rowIndex,colIndex)->getU1() - getPrimitivePtr(rowIndex,colIndex-1)->getU1();
	else // In the interior, use central diff
		dU1dv = (getPrimitivePtr(rowIndex,colIndex+1)->getU1() - getPrimitivePtr(rowIndex,colIndex-1)->getU1()) / 2;

	return dU1dv;
}

double M3DQuadFigure::computeDR0du(int rowIndex, int colIndex)
{
	if (rowIndex < 0 || rowIndex > numRows - 1 || colIndex < 0 || colIndex > numColumns - 1)
	{
		std::cout << "Invalid row or column: row " << rowIndex << ", column " << colIndex << std::endl;
	...
	double dr0du;
	if (rowIndex == 0) // On left edge, use forward diff
		dr0du = dynamic_cast<M3DQuadPrimitive*>(getPrimitivePtr(rowIndex+1,colIndex))->getR0() -
				dynamic_cast<M3DQuadPrimitive*>(getPrimitivePtr(rowIndex,colIndex))->getR0();
	else if (rowIndex == numRows - 1) // On right edge, use backward diff
		dr0du = dynamic_cast<M3DQuadPrimitive*>(getPrimitivePtr(rowIndex,colIndex))->getR0() -
				dynamic_cast<M3DQuadPrimitive*>(getPrimitivePtr(rowIndex-1,colIndex))->getR0();
	else // In the interior, use central diff
		dr0du = (dynamic_cast<M3DQuadPrimitive*>(getPrimitivePtr(rowIndex+1,colIndex))->getR0() -
				 dynamic_cast<M3DQuadPrimitive*>(getPrimitivePtr(rowIndex-1,colIndex))->getR0()) / 2;

	return dr0du;
}
double M3DQuadFigure::computeDR1du(int rowIndex, int colIndex)
{
	if (rowIndex < 0 || rowIndex > numRows - 1 || colIndex < 0 || colIndex > numColumns - 1)
	{
		std::cout << "Invalid row or column: row " << rowIndex << ", column " << colIndex << std::endl;
	...
	double dr1du;
	if (rowIndex == 0) // On left edge, use forward diff
		dr1du = dynamic_cast<M3DQuadPrimitive*>(getPrimitivePtr(rowIndex+1,colIndex))->getR1() -
				dynamic_cast<M3DQuadPrimitive*>(getPrimitivePtr(rowIndex,colIndex))->getR1();
	else if (rowIndex == numRows - 1) // On right edge, use backward diff
		dr1du = dynamic_cast<M3DQuadPrimitive*>(getPrimitivePtr(rowIndex,colIndex))->getR1() -
				dynamic_cast<M3DQuadPrimitive*>(getPrimitivePtr(rowIndex-1,colIndex))->getR1();
	else // In the interior, use central diff
		dr1du = (dynamic_cast<M3DQuadPrimitive*>(getPrimitivePtr(rowIndex+1,colIndex))->getR1() -
				 dynamic_cast<M3DQuadPrimitive*>(getPrimitivePtr(rowIndex-1,colIndex))->getR1()) / 2;

	return dr1du;
}
double M3DQuadFigure::computeDR0dv(int rowIndex, int colIndex)
{
	if (rowIndex < 0 || rowIndex > numRows - 1 || colIndex < 0 || colIndex > numColumns - 1)
	{
		std::cout << "Invalid row or column: row " << rowIndex << ", column " << colIndex << std::endl;
	...
	double dr0du;
	if (colIndex == 0) // On left edge, use forward diff
		dr0du = dynamic_cast<M3DQuadPrimitive*>(getPrimitivePtr(rowIndex,colIndex+1))->getR0() -
				dynamic_cast<M3DQuadPrimitive*>(getPrimitivePtr(rowIndex,colIndex))->getR0();
	else if (colIndex == numColumns - 1) // On right edge, use backward diff
		dr0du = dynamic_cast<M3DQuadPrimitive*>(getPrimitivePtr(rowIndex,colIndex))->getR0() -
				dynamic_cast<M3DQuadPrimitive*>(getPrimitivePtr(rowIndex,colIndex-1))->getR0();
	else // In the interior, use central diff
		dr0du = (dynamic_cast<M3DQuadPrimitive*>(getPrimitivePtr(rowIndex,colIndex+1))->getR0() -
				 dynamic_cast<M3DQuadPrimitive*>(getPrimitivePtr(rowIndex,colIndex-1))->getR0()) / 2;

	return dr0du;
}
double M3DQuadFigure::computeDR1dv(int rowIndex, int colIndex)
{
	if (rowIndex < 0 || rowIndex > numRows - 1 || colIndex < 0 || colIndex > numColumns - 1)
	{
		std::cout << "Invalid row or column: row " << rowIndex << ", column " << colIndex << std::endl;
	...
	double dr0du;
	if (colIndex == 0) // On left edge, use forward diff
		dr0du = dynamic_cast<M3DQuadPrimitive*>(getPrimitivePtr(rowIndex,colIndex+1))->getR1() -
				dynamic_cast<M3DQuadPrimitive*>(getPrimitivePtr(rowIndex,colIndex))->getR1();
	else if (colIndex == numColumns - 1) // On right edge, use backward diff
		dr0du = dynamic_cast<M3DQuadPrimitive*>(getPrimitivePtr(rowIndex,colIndex))->getR1() -
				dynamic_cast<M3DQuadPrimitive*>(getPrimitivePtr(rowIndex,colIndex-1))->getR1();
	else // In the interior, use central diff
		dr0du = (dynamic_cast<M3DQuadPrimitive*>(getPrimitivePtr(rowIndex,colIndex+1))->getR1() -
				 dynamic_cast<M3DQuadPrimitive*>(getPrimitivePtr(rowIndex,colIndex-1))->getR1()) / 2;
