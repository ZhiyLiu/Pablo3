...
// 	if ( numBasisVectors > 0 )
// 	{
// 	
// 	vector <double *> _basisVector;
// 	
// 	  if( ! readShapeBasisVectors( _basisVector, numBasisVectors, basisVectorLen) ) 
// 	  {
// 	    cout << "Error in M3DObjectFile::readCPNSStats() ! basisVector not read correctly from registry << endl";
// 	    delete cpnsStats ;
// 	    cpnsStats = NULL ;
// 	    return NULL;
// 	  }
// 	  else if( ! cpnsStats->setBasisVectors( _basisVector) )
// 	  {
// 	    cout << "Error in M3DObjectFile::readCPNSStats() ! basisVector not set correctly << endl";
// 	    delete cpnsStats ;
// 	    cpnsStats = NULL ;
// 	    return NULL; 
// 	  }
// 	  
// 	  cout << "Basis Vectors Read" << endl;
// 	}
...
M3DPNSTransform * M3DObjectFile::readPNSShape() {
	...
	int numBasisVectors = registry.getIntValue("CPNSStats.PNSShape.basisVectorNumber", -1) ;
	int basisVectorLen = registry.getIntValue("CPNSStats.PNSShape.basisVectorLength" , -1) ;
	...
	pnsShape->setNumBasisVectors(numBasisVectors);
	pnsShape->setBasisVectorLength(basisVectorLen);
	...
	{
	  vector <double *> _basisVector;
	  readShapeBasisVectors( _basisVector, numBasisVectors, basisVectorLen );
	  pnsShape->setBasisVectors( _basisVector );
	}
...
bool M3DObjectFile::readShapeBasisVectors( vector <double *> & _basisVectors, int numBasisVectors, int basisVectorLen )
{
    char fieldName[100] ;
    
    double * thisBasisVector ;
    
    for( int i = 0; i < numBasisVectors; i++ )
    {
      sprintf( fieldName, "CPNSStats.PNSShape.basisVector[%d]", i) ;
      
      thisBasisVector = registry.getDoubleArray( (const char*) fieldName, & basisVectorLen );
      
      if( thisBasisVector == NULL )
      {
	cout << "Error in M3DObjectFile::readShapeBasisVectors() ! basisVector[" << i << "] could not be read from registry" << endl;
	return(0);
      }
      
      _basisVectors.push_back( thisBasisVector );
