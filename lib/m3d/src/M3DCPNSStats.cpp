...
bool M3DCPNSStats::setBasisVectors(vector <double *> & _basisVector)
{
  if (basisVector.empty() ) 
  {
    basisVector.reserve( nBasisVectors );
    for (int i = 0; i < nBasisVectors; i++)
      basisVector.push_back( NULL );
  }
  
  double * thisBasisVector = NULL;
  
  for (int i = 0; i < nBasisVectors; i++)
  {
    thisBasisVector = new double[basisVectorLength];
    
    for (int j = 0; j < basisVectorLength; j++)
      thisBasisVector[j] = *( _basisVector[i] + j );
    
    basisVector[i] = thisBasisVector;
    thisBasisVector = NULL;
  }
...
bool M3DCPNSStats::setNumBasisVectors( int _numBasisVectors )
{
  this->nBasisVectors = _numBasisVectors;
  
  return(1);
}

bool M3DCPNSStats:: setBasisVectorLength ( int _basisVectorLength )
{
  this->basisVectorLength = _basisVectorLength;
...
M3DFigure * M3DCPNSStats::convertECompToSRepFigure(double *eComp, Vector3D meanOfPDM ) {
	...
	int nEndAtoms = 2*this->nAtomRows + 2*(this->nAtomCols - 2) ;
	int nStdAtoms = nAtoms - nEndAtoms;

	int nPositions = 3 * (nEndAtoms + nStdAtoms);
	int nRadii = 3*nEndAtoms + 2*nStdAtoms;
	
	int nDimensions = this->PNSShape->getNDimensions(); // number of dimensions of the PNS shape vector
	// -------------------------- recover positions  ---------------------------
	...
	double * ePositions = new double[ nDimensions ];
	
	//cout << "nPositions: " << nPositions << endl;
	//cout << "nDim: " << this->PNSShape->getNDimensions() << endl;
	//cout << "nePositions: " << 3*nAtoms - 1 << endl;
	...
	double scaleShapeThis = scaleShape * exp( eComp[ nDimensions ] / scaleShape ) ;
	...
	for( int i = 0 ; i < nAtoms ; i++ ) {
		positions[ 3*i + 0 ] += meanOfPDM.getX() ;
		//std::cout << positions[3*i + 0] << ", ";
		positions[ 3*i + 1 ] += meanOfPDM.getY() ;
		//std::cout << positions[3*i + 1] << ", ";
		positions[ 3*i + 2 ] += meanOfPDM.getZ() ;
		//std::cout << positions[3*i + 2] << ", ";
	}
	...
	for( int ns = 0 ; ns < nSpokes ; ns ++ )
		spokeRadii[ns] = scaleSpoke[ns] * exp( eComp[ nDimensions + 1 + ns ] / scaleSpoke[ns] ) ;
	...
	for( int ns = 0 ; ns < nSpokes ; ns ++ ) {
		...
		for( int j = 0 ; j < 2 ; j++ )
		{
			eSpokeThis[ j ] = eComp[ nDimensions + 1 + nSpokes + 2*ns + j ] ;
			eSpokeThis[ j ] = eSpokeThis[ j ] / scaleSpoke[ns];
		}
...
/*M3DFigure * M3DCPNSStats::convertECompToSRepFigure2(double * CPNSScores, Vector3D meanOfPDM )
{
	int nTotalAtoms = nAtomCols * nAtomRows;
	int nEndAtoms = 2*nAtomRows + 2*(nAtomCols-2);
	int nStdAtoms = nTotalAtoms - nEndAtoms;
	
	int nTotalPositions = 3*(nEndAtoms + nStdAtoms);
	int nTotalRadii = 3*nEndAtoms + 2*nStdAtoms;
	
	// 1. Construct X (hub positions)

	int dimZShape = this->PNSShape->getNDimensions();
	
	double * zShape = new double[ dimZShape - 1 ];
	
	for (int i = 0; i < dimZShape - 1; i++)
	  zShape[i] = CPNSScores[i] / scaleShape;
	
	double zSizePDM = CPNSScores(dimZShape - 1);
	double sizePDMOverall = scaleShape * exp( zSizePDM / scaleShape );
}*/


M3DObject * M3DCPNSStats::eigenmodeDeformMean( double * scores, M3DObject * origObject, int nModes ) {

	// Deform a mean s-rep along its eigenmodes
	...
	// Compute alignment from CPNS mean to incoming object, which should be aligned to image data.
	// This is necessary because the mean+eigenmodes are in the pre-CPNS alignment space and need
	// to be realigned to the input object
        
	if (!alignmentComputed)
	{
	  baseObj = origObject->clone();
	  double * EComp_mean = new double [ eigenVectorLength ] ;

	  for( int i = 0 ; i < eigenVectorLength ; i++ )
		  EComp_mean[i] = 0.0 ;

	  cout << "Testing mean" << endl;
	  M3DFigure * testFig = convertECompToSRepFigure( EComp_mean, this->meanShape );
	  cout << "Mean done" << endl;

	  M3DObject * testObj = origObject->clone();
	  testObj->setFigurePtr(0, testFig);
			transformToMean = MatchUtility::initializeObjectToObject( testObj, origObject, 7, -1);
	  alignmentComputed = true;

	  delete [] EComp_mean;
	}

	// If any of the scores are outside [-3, +3] std devs, then bring them within that bracket
	...
	// multiply each score (# std devs) by std dev
	...
	// build Z matrix (EComp = Euclidean COMPosite matrix)

	int nAtoms = nAtomRows * nAtomCols ;

	double * EComp = new double [ eigenVectorLength ] ;

	for( int i = 0 ; i < eigenVectorLength ; i++ )
		EComp[i] = 0.0 ;

	for( int ne = 0 ; ne < nModes ; ne++ ) {
		for( int i = 0 ; i < eigenVectorLength ; i++ )
	...
	M3DObject * resultObj = origObject->clone() ;
        
	// construct new s-rep from Z matrix
	M3DFigure * newFig = convertECompToSRepFigure( EComp, this->meanShape ) ;
	
    newFig->setName("newFig");

    M3DObject * tempObj = origObject->clone();
	tempObj->setFigurePtr( 0, newFig ) ;
    
    // apply transformation to get back into alignment with initialization
    SimilarityTransform3D transform = 
            MatchUtility::initializeObjectToObject(tempObj, baseObj, 7, -1) ;
    
	newFig->applySimilarityAboutCOG(&transformToMean);
	resultObj->setFigurePtr( 0, newFig);
    //resultObj->getFigurePtr(0)->applySimilarityAboutCOG(&globalTransform);
