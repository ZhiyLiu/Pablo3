...
bool M3DPNSTransform::setBasisVectors(vector <double *> & _basisVector)
{
  if (basisVector.empty() ) 
  {
    basisVector.reserve( numBasisVectors );
...
// 	rotMatTranspose.clear() ;
	...
	if (this->numBasisVectors > 0)
	{	  
	  if (this->nDims + 1 != numBasisVectors)
	    cout << "Error in M3DPNSTransform::convertE2S(): Basis Vector/Data Length Mismatch" << endl;
	  
	  double * S2 = new double[ basisVectorLength ] ;
	  ...
	  for (int j = 0; j < basisVectorLength; j++)
	  {
	    for (int i = 0; i < numBasisVectors; i++)
	    {
	      basisElement = *(basisVector[i] + j);
	      temp += basisElement * S[i];
	    }
	    S2[j] = temp;
	    temp = 0.0;
	  }
