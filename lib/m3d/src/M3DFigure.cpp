...
void M3DFigure::displayInterpolatedBoundary(int level)
{
    /*glBegin(GL_QUADS);
...
Vector3D M3DFigure::getInterpolatedBoundaryPoint(double u, double v, int side)
{
    M3DInterpolater *interp = new M3DInterpolater(this->clone());
    //M3DSpoke* spoke = interp->interpolateSpoke_new(this->clone(),u,v,side);
    M3DSpoke spoke = interp->interpolateSpoke(this->clone(), u, v, side);
//    if (u > 2 && u <= 3 && v > 2 && v <= 3)
    // if(side == 0)
    spoke.draw();
...
M3DSpoke M3DFigure::getInterpolatedSpoke(double u, double v, int side)
{
  M3DInterpolater *interp = new M3DInterpolater(this->clone());
  M3DSpoke spoke = interp->interpolateSpoke(this->clone(), u, v, side);
  return spoke;
}
void M3DFigure::draw2D(std::vector<M3DSpoke>& spokes)
{
...
double g[3] = {0.0, 1.0, 0.0};
double b[3] = {0.0, 0.0, 1.0};
void M3DFigure::draw2D(std::vector<M3DSpoke>& spokes, int figureID)
{
    if(figureID < 0 || figureID > 2)
    {
        return;
    }
    ...
    for(int i = 0; i < spokes.size(); ++i)
    {
        spokes[i].draw(r[figureID], g[figureID], b[figureID]);
    }

}
void M3DFigure::interpolate2D(std::vector<M3DSpoke>& spokes, int side)
{
//    if(style == M3D_SOLID)
    {
        M3DInterpolater *interp = new M3DInterpolater(this->clone());

        int interpolateLevel = 3;

        // the output spokes contains both interpolated spokes and primary spokes
        interp->interpolate2D(this->clone(), interpolateLevel, side, spokes);
         // draw2D(upSpokes);
         // draw2D(downSpokes);
    }
}