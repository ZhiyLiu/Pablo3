...
void M3DPrimitiveRenderer::render()
{
	...
	if (M3DObjectRenderer::atomVectorVisibility()) {
        ...
        // Zhiyuan Liu: get more recognizable spokes
		...
		if(tubePrimitive != NULL) {
			...
			// Draw Y0 vector
            //glColor3f(0.0f, 1.0f, 1.0f); // for black background
            glColor3f(0.0f, 0.5f, 1.0f); // for white background
			glVertex3d(center.getX(), center.getY(), center.getZ());
			glVertex3d(y0.getX(), y0.getY(), y0.getZ());

			// Draw Y1 vector
            //glColor3f(1.0f, 0.3f, 1.0f); // for black background
            glColor3f(1.0f, 0.0f, 1.0f); // for white background
			glVertex3d(center.getX(), center.getY(), center.getZ());
