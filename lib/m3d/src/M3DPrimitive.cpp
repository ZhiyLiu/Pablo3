...
M3DSpoke M3DPrimitive::getSpoke(int side)
{
    Vector3D pos, dir;
    double r;
    pos = getX();
    ...
    if(side == 0)
    {
        dir = dynamic_cast<M3DQuadPrimitive*>(this)->getU0();
        r = dynamic_cast<M3DQuadPrimitive*>(this)->getR0();
    }
    else if(side == 1)
    {
        dir = dynamic_cast<M3DQuadPrimitive*>(this)->getU1();
        r = dynamic_cast<M3DQuadPrimitive*>(this)->getR1();
    }
    else if(side == 2)
    {
        M3DQuadEndPrimitive* endP = dynamic_cast<M3DQuadEndPrimitive*>(this);
        if(endP != NULL)
        {
            dir = dynamic_cast<M3DQuadPrimitive*>(this)->getUEnd();
            r = endP->getREnd();
        }
        else
        {
            std::cerr<< "The primitive is not end primitive, thus side could only be 0 or 1" << std::endl;
            assert(false);
        }
    }

    return M3DSpoke(pos, dir, r);
}
