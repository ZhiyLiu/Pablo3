...
public:
    ...
    // Zhiyuan : this function is for visualizing 2D s-rep
    // the first draw connect end of spokes as implied boundary
    void draw2D(std::vector<M3DSpoke>& spokes);

    // the second draw with figureID draw spokes of different color relating to each figure
    void draw2D(std::vector<M3DSpoke>& spokes, int figureID);
    void interpolate2D(std::vector<M3DSpoke>& spokes, int side);
...
//	int	findLandmarkAtomIndexByName(const char *name, int instance = -1);
	...
	void displayInterpolatedBoundary(int level);
	Vector3D getInterpolatedBoundaryPoint(double u, double v, int side);
	M3DSpoke getInterpolatedSpoke(double u, double v, int side);
