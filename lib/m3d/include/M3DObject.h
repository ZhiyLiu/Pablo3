...
public:
    ...
    // Zhiyuan Liu: Read in selection result from matlab
    void readSelectionResult(const std::string& filename);

    // for the use of linking structure
    bool initLinkingStructure = false;
    char* outputLinkingStructurePath = NULL;
    std::string outputLinkingFile;
    std::vector<int> selectedLinks;

    void setOutputLinkingPath(const std::string& linkingPath) {outputLinkingFile = linkingPath;};
    void renderLinkingStructure(bool isAfterSelection = true);

    // compute delta(y, y') for linking structure, output the spoke at position x
    // x is parameter on s-rep of fig
    // return delta
    double computeDelta2D(M3DFigure& fig, M3DSpoke& targetS, double x, M3DSpoke& output);
    double computeDelta(M3DSpoke& y_prime, M3DSpoke& y);
    // check if the two spokes are possibly linked to each other
    // input: target is the home spoke who is looking for linking correspondence
    bool isLinkPossible(M3DSpoke& ref, M3DSpoke& target);
    // find better spoke according to their delta to target spoke
    // input: target spoke
    // input: spoke 1 and spoke 2
    // output: put the better spoke in output
    // return the smaller delta
    double findBetterLinkedSpoke(M3DSpoke& targetS, M3DSpoke& spoke1, M3DSpoke& spoke2, M3DSpoke& output);

    void saveLinkingStructure(std::vector<std::vector<M3DSpoke> > &linkingStructure);

    // organize an array of spokes into a figure
    // upSpokes contains 2 crest spokes, so does downSpokes
    void convertSpokes2Figure(std::vector<M3DSpoke> &upSpokes, std::vector<M3DSpoke> &downSpokes, M3DQuadFigure& output);
...
protected:
	...
	M3DObject * that;	// A duplicate of this, sets by class M3DObjectFile
	int * counter;
...
private:
    void drawAllSpokes(std::vector<M3DSpoke> &homeFig);
