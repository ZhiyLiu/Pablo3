...
public:
    ...
    // Jared -- added derivative calculations
    // 	This seems to be as far down as these can go, as individual atoms won't know about neighbors
    // 	The indices assume u = rows and v = columns

    Vector3D computeDXdu(int rowIndex, int colIndex);
    Vector3D computeDXdv(int rowIndex, int colIndex);

    Vector3D computeDU0du(int rowIndex, int colIndex);
    Vector3D computeDU1du(int rowIndex, int colIndex);
    Vector3D computeDU0dv(int rowIndex, int colIndex);
    Vector3D computeDU1dv(int rowIndex, int colIndex);

    double computeDR0du(int rowIndex, int colIndex);
    double computeDR1du(int rowIndex, int colIndex);

    double computeDR0dv(int rowIndex, int colIndex);
    double computeDR1dv(int rowIndex, int colIndex);


    // compute derivatives on 2d s-reps
    Vector3D computeDX(int row);
