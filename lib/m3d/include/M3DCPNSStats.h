...
public:
	...
	bool setBasisVectors (vector <double *> & _basisVector );
	bool setNumBasisVectors (int _numBasisVectors);
	bool setBasisVectorLength (int _basisVectorLength);
	...
	M3DObject * eigenmodeDeformMean( double * scores, M3DObject * origObject, int nModes ) ;
	...
	M3DFigure * convertECompToSRepFigure2(double * eComp, Vector3D meanOfPDM ) ;
...
protected:
	...
	int nBasisVectors;
	int basisVectorLength;
	
	vector <double *> basisVector ;

	double *			eigenValue ;		// eigenValue[i] = i-th eigenvalue

	SimilarityTransform3D transformToMean;
	
        M3DObject * baseObj;
	
	bool alignmentComputed;
