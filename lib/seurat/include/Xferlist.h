...
public:
	...
	MyPoint2D(const double v)
	{
		x=v; y=v;
	}
	MyPoint2D(const double xx, const double yy)
	{
		x=xx; y=yy;
	}
	...
	friend MyPoint2D operator*(const double a, const MyPoint2D &A)
	{
		return MyPoint2D(A.x*a, A.y*a);
	}
	friend MyPoint2D operator*(const MyPoint2D &A, const double a)
	{
		return MyPoint2D(A.x*a, A.y*a);
	}
	// dot product
	friend double operator*(const MyPoint2D &A, const MyPoint2D &B)
	{
	...
	MyPoint2D operator/(const double a)
	{
	...
	//friend MyPoint2D operator^(const MyPoint2D &A, const double a)
	//{
	//	return MyPoint2D((double)pow(A.x, a), (double)pow(A.y, a));
	//}
	...
	//double Distance(const MyPoint2D &A)
	//{
	//	return (double)sqrt((A.x-x)*(A.x-x)+(A.y-y)*(A.y-y));
	//}
	double DistanceSquare(const MyPoint2D &A)
	{
		return ((A.x-x)*(A.x-x)+(A.y-y)*(A.y-y));
	}

	double CircularDistance(const MyPoint2D &A)
	{
		double vDelta, tDelta;
	...
	static void Normal(double *n, double* v0, double* v1, double* v2)
	{
	...
	void Normalize(void)
	{
		double mag;//=Mag();		Uninitialized variable!
		if(mag>0)
		{
	//		x/=mag;
	//		y/=mag;
	...
	{
		double mag;//=A.Mag();		Uninitialized variable!
		if(mag>0)
