...
/********************************************************************************/
#ifndef POINTLIST_SERVERB_H_
#define POINTLIST_SERVERB_H_
#include <queue>
#include <set>
#include "Xferlist.h"
#include "LinAlg.h"
...
//	static std::vector<ThallCode::Pointlist_server2 *> tileLists;
	...
	//////////////////////////////////////////////////////////////////////////////////////////////////////////////
	double create_plist(M3DFigureTreeNode * node);
	double linearize(M3DFigureTreeNode * node);
	...
	void RegularizeIntersectionCurveUVT(double uMaxM, double vMaxM, double uMaxS, double vMaxS, Bpoint2 *curv, int pNum);
	...
	void RegularizeIntersectionCurve(double vMaxS, Bpoint2 *newC, Bpoint2 *oldC, int *newPairs, int *oldPairs, int *pNum);

	bool SamePoint(double epsilon, Bpoint p1, Bpoint p2);
	bool SamePoint(double epsilon, Bpoint2 p1, Bpoint2 p2);

	bool SamePointByDistance(double epsilon, Bpoint2 p1, Bpoint2 p2);
	bool SamePointByDistanceSquare(double epsilon, Bpoint2 p1, Bpoint2 p2);

	void ConvertCoordinates(MyPoint2D *c2D, Bpoint2 *c, double mU, double mV, int pNum);
	void DilatePoint(Bpoint2 *pnt, double maxU, double maxV, double deltaU, double deltaV);
	void CalculateDilationDirection(MyPoint2D *c2D, Bpoint2 *c, double mU, double mV, int pNum);
	bool DilateCurve(Pointlist_server2 *pList, Bpoint2 *curv, int pNum, double dt, int steps);
	//*/
	...
	bool ConvertUVT2XY_Bottom(int type, int area, Bpoint2 curv, MyPoint2D *curv2D, double maxU, double maxV);
	bool ConvertUVT2XY_Crest(int type, Bpoint2 curv, MyPoint2D *curv2D, double maxU, double maxV);
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	void glRender(int subLvl, int polyDex, RenderStyle renderMode, double *c);
	void glRender(int vNum, int fNum, Bpoint2 *vList, PindexList *fList, RenderStyle renderMode, double *c);
	void glRenderCurve(int startPDex, int endPDex, Bpoint2 *pntList, double *c);
	void glRenderSegments(int startPDex, int endPDex, Bpoint2 *pntList, double *c);

	void glRenderBlended(int subLvl, RenderStyle renderMode);
	void glRenderPoint(Bpoint2 pnt, double *c);
...
private:
	...
	double MINIMAL_SEGMENT_LENGTH_SQUARE;
	...
	void IntersectMeshes(double uCutOff, int nTiles1, Bpoint *tile1, int nTiles2, Bpoint *tile2, \
	...
	bool FindIntersectionCurveFromSegments(Bpoint2 *interPList2, int *interPPair2, int *intPNum, \
		Bpoint2 *interPList, int *interPPair, double EPSILON2);
