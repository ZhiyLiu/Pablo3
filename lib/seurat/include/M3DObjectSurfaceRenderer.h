...
public:
	...
	// range [1, 4].
	M3DObjectSurfaceRenderer(M3DObject * obj = nullptr, int lvl = 1);
	virtual ~M3DObjectSurfaceRenderer();

    Diatomgrid * convertFigure(M3DFigure * fig);
	...
	Pointlist_server2 * getPList(int figureId);
	...
	void drawDots(double * spot_list, int numSpots, double size = 0.0,
		...
		const float * color = nullptr, int numBigSpot = -1);

	void renderFigureFromControlPoints(int numverts, int numfaces,
		Bpoint *pntlist, PindexList *faces);
...
private:
	...
	Pointlist_server2 * computeFigure(int index,
