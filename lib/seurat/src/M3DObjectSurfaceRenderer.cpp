...
Diatomgrid * M3DObjectSurfaceRenderer::convertFigure(M3DFigure * _fig)
{
    Diatomgrid * grid;
    M3DPrimitive * prim;
    Diatom * atom;
    ...
    grid->dlist = new Diatom[grid->rows * grid->cols];
    ...
    for(i = 0; i < grid->cols; i++)
    {
        for(j = 0; j < grid->rows; j++)
        {
            ...
            atom->set_p(DbVector3(prim->getX().getX(), prim->getX().getY(), prim->getX().getZ()));
            atom->set_q(q2);
...
void M3DObjectSurfaceRenderer::renderFigure(int figureId)
{
    ...
    // Zhiyuan Liu: temporary solution for visualizing 2D srep
    int numCols = dynamic_cast<M3DQuadFigure*>(figure)->getColumnCount();
    int numRows = dynamic_cast<M3DQuadFigure*>(figure)->getRowCount();
    if(numCols == 1 && numRows > 1)
    {
        ...
        figure->draw2D(upSpokes);
        figure->draw2D(downSpokes);
        return;
    }
	...
	if (boundary != NULL)
	{
		...
		switch (surfaceStyle)
		{
			...
			case M3D_SOLID:
				...
				thisPList->glRenderSubdivTiles(0, figure, fastInterp);
				//interp->displayInterpolatedBoundary(figure, 0);
				//figure->displayInterpolatedBoundary(0);
...
Pointlist_server2 * M3DObjectSurfaceRenderer::computeFigure(int figureId,
...
Pointlist_server2 * M3DObjectSurfaceRenderer::getPList(int figureId) {
    int numFigures;
...
{
	list = new Pointlist_server2 *[length];
				...
				double cull_distance, bool complete_figures, int level)
{
	...
	Pointlist_server2 ** pLists =
		(Pointlist_server2 **) figure_list->pointLists();
...
void M3DObjectSurfaceRenderer::renderFigureFromControlPoints
    (int numverts, int numfaces, Bpoint *pntlist, PindexList *faces)
{
   Pointlist_server2 * thisPList;
