...
int vtkSRepInterpolateMedialSpokesHermite::RequestData(vtkInformation* request,
    ...
    vtkSmartPointer< vtkDataArray > interpolatedlabelarraysrep = (vtkDataArray*)vtkDataArray::CreateArray(VTK_DOUBLE);
    vtkSmartPointer< vtkDataArray > interpolatedlabelarraysurface = (vtkDataArray*)vtkDataArray::CreateArray(VTK_DOUBLE);
    ...
    for(unsigned i = 0; i < vectcellsids.size(); i++){
        ...
        vtkCell* quad = input->GetCell(vectcellsids[i]);
    ...
    for(unsigned i = 0; i < vectcellsids.size(); i++){
        ...
        if(m_AtomId != -1){
            vtkCell* cell = input->GetCell(vectcellsids[i]);
        ...
        for(double u = u0, curru = 0; u <= uend; u += stepu, curru++){
            ...
            for(double v = v0 ; v <= vend; v += stepv){
                ...
                interpolatedlabelarraysrep->InsertNextTuple1(floor(interpolatedlabel));
        ...
        for(unsigned j = 0; j < srepinterpolatedids.size() - 1; j++){
            for(unsigned k = 0; k < srepinterpolatedids[j].size() - 1; k++){
...
void vtkSRepInterpolateMedialSpokesHermite::GetHermiteMatrixRadius(vtkSRep* input, vtkIdType id0, vtkIdType id1, vtkIdType id2, vtkIdType id3, vtkIdType side, VNLType **H){
    ...
    /*vtkSmartPointer<vtkDataArray> curvaturearray = input->GetPointData()->GetArray(CURVATURE_ARRAY);
    VNLType k0(1);
    k0.fill(100);
    VNLType k1(1);
    k1.fill(100);
    ...
    if(curvaturearray){
        double curvtuple[1];
        curvaturearray->GetTuple(id0, curvtuple);
        k0[0] = curvtuple[0];

        curvaturearray->GetTuple(id1, curvtuple);
        k1[0] = curvtuple[0];

        curvaturearray->GetTuple(id2, curvtuple);
        k2[0] = curvtuple[0];

        curvaturearray->GetTuple(id3, curvtuple);
        k3[0] = curvtuple[0];
    }*/
