	...
	for (int i = 0; i < capacity; i++) {
		if (data[i]) {
			if ((data[i]->key)[0] == '=')
				...
				if (array != NULL) {
                	switch(type) {
                      /*case DoubleArray: { cout << type << endl; delete [] ((double *) array); break; }
                      case FloatArray: { cout << type << endl; delete [] ((float *) array); break; }
                      case IntArray: { cout << type << endl; delete [] ((int *) array); break;}*/
                    delete[] array;
                    cout << "here" << endl;
