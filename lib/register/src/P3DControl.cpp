			...
			for (int j = 0; j < MAX_NUM_FIG_MATCH_RESULTS; j++) {
				...
				switch (index) {	// Why not just print the "short name"?
					...
					case BpFigureImageVertexMatch:		cout << " Crest vertex" ;
						break ;
                   // Chong
					case BpAtomEndSpokeDirectionalConsistency: cout << " End atom spoke direction";
						break;
...
bool P3DControl::binaryPabloSRepNormalAdjustments()
{
	M3DQuadFigure * figure = dynamic_cast<M3DQuadFigure*>(object->getFigurePtr(0)); // This should be changed to deal with multiple objects

	int num_prims = figure->getPrimitiveCount();

	std::cout << num_prims << std::endl;

	for (int i = 0; i < num_prims; i++)
	{
		M3DPrimitive * prim = figure->getPrimitivePtr(i);
		if (prim->type() == 0) // 0 = Standard, 1 = End Primitive; Right now, we don't adjust anything for end atoms
		{
			match->doSpokeNormalAdjustment(object, 0, i, 0, 0);
			match->doSpokeNormalAdjustment(object, 0, i, 1, 0);
		}
	}
...
char* calcAntialiasedDMFilename(char* _imageFilename) {
    ...
    string ddmFilenameTemp(ddmFilename);
    ddmFilenameTemp.append(".mhd");
    ifstream ifile(ddmFilenameTemp.c_str());

    if(ifile){
        strcat(ddmFilename, ".mhd");
    }else{
...
int P3DControl::runThinPlateSpline(Registry &registry)
{
	...
	std::vector<Pointlist_server2*> pList(numModels);
	...
	double dist;
	int multiplicity;
	DbVector3 nearpointXYZ;
	DbVector3 nearpointUVT;
	Vector3D coord;
	...
	if( registry.getIntValue("BackProject", 0) == 0 ) {
		...
		for( int m = 0; m != numModels; ++m) {
			...
			for( p = 0; p != numPoints; ++p ) {
				if( !pList[m]->thisMesh->isEndMedcoord(Medcoord(surfacePointList[p])) ) {
...
//		object->getCPNSStats()->setMeanShape(newMean);
	...
	newMean[2] = meanZ;
        ...
        // Now compute new scale of pdm

        for (int i = 0; i < numAtoms; i++)
        {
            positions[3*i + 0] -= newMean[0];
            positions[3*i + 1] -= newMean[1];
            positions[3*i + 2] -= newMean[2];

        }

        double newScaleShape = 0.0;

        for (int i = 0; i < 3*numAtoms; i++)
            newScaleShape += positions[i] * positions[i];

        newScaleShape = sqrt(newScaleShape);

	/*if (object->getCPNSStats() != NULL)
        {
		object->getCPNSStats()->setMeanShape(newMean);
                object->getCPNSStats()->setScaleShape(newScaleShape);
        }*/
...
#define DEBUG_DIBYENDU_WRITE_DIST
	...
	std::cout << "norm: " <<  (int) tuningWt(SRepDoNormalAdjustments) << std::endl;

	if ((int) tuningWt(SRepDoNormalAdjustments) == 1)
		binaryPabloSRepNormalAdjustments();
	...
	*lobj = *object;
	...
	// Jared 2014/11/18 - should now work even if no optimization is done
//	if (match->atomPredictor == NULL)
		match->setUpAtomPredictor(0);
...
pList = new Pointlist_server2[numFigures];
...
void P3DControl::showSelectedLinks(const char * filename)
{
    ...
    if(object->selectedLinks.empty())
    {
        object->selectedLinks = idList;
    }
    else
    {
        object->selectedLinks.clear();
    }
...
bool P3DControl::cpnsDeform(std::vector<double> & vals)
{
	...
	M3DObject * newObject = cpnsStats->eigenmodeDeformMean( scores, object, nEigenmodes ) ;
...
// Zhiyuan Liu
// compute Linking structure for 2D srep
void P3DControl::computeLinkingStructure()
{
    if(object == NULL)
    {
        return;
    ...
    object->initLinkingStructure = true;
    object->outputLinkingStructurePath = globalControl->readString(ModelDirectory);
    object->setModified(true);

    return;
}

void P3DControl::computeGeoEntropy()
{
    if(object == NULL)
    {
        return;
