...
// Here, object1 is moving, object2 is fixed
SimilarityTransform3D MatchUtility::initializeObjectToObject(M3DObject * object1,
	M3DObject * object2, int moments, int verbose)
{
    SimilarityTransform3D momTransform;

	if (moments) {
		Vector3D figureCenter, figureCenter2, initialTranslation;
		double figureVolume, figureVolume2, initialScale;
		Quat figureRotation, figureRotation2, initialRotation;

		MethodOfMoments::objectMoments(object1, figureVolume, figureCenter, figureRotation);
		//figureCenter = object1->getCOG();
		
		MethodOfMoments::objectMoments(object2, figureVolume2, figureCenter2, figureRotation2);
                //figureCenter2 = object2->getCOG();
		
		// Translation is just the difference of the two center's
		initialTranslation = figureCenter2 - figureCenter;
		...
		// Note here that the SCALE is the CUBE ROOT of the RATIO of VOLUMES
		initialScale = pow(figureVolume2/figureVolume, 1.0/3.0);

		// Determine the smallest angle rotation we can make to align the figure and image cov's
		MethodOfMoments::leastAngleRotation(figureRotation2, figureRotation, initialRotation);
