...
void Match::definePLists(M3DObject * object, int subdivLevel)
{
	...
	pList = new Pointlist_server2*[pListCount];
	...
	for (i = 0; i < figureCount; i++) {
		...
		pList[i] = new Pointlist_server2;
		pList[i]->init(xferList);
		pList[i]->ComputeSubdivPointCloud(subdivLevel);

		pList[figureCount + i] = new Pointlist_server2;
...
double Match::computeSpokeBinaryImageMatch(M3DObject * targetObject, int figureId, int atomId, int spokeId, int subdivLevel)
{
	...
	Vector3D tempPoint = quadAtom->getX() ;
	...
	switch( spokeId ) {
		...
		case 2 :    if( quadAtom->type() == M3D_END_PRIMITIVE )
...
// 	Jared - This function was created to adjust interior spokes to attempt to make them more orthogonal to the boundary
// 		This is a workaround for problems when running an optimization where the only penalty is orthogonality, as the
// 		spokes move too far from their starting points.
//
// 	This function approximates the tangent plane at the current spoke end and moves the spoke to be orthogonal to it
// 		Assuming the tangent plane is a good approximation for the boundary, this should be reasonably accurate.

void Match::doSpokeNormalAdjustment( M3DObject * targetObject, int figureId, int atomId, int spokeId, int subdivLevel )
{
	M3DFigure * figure = targetObject->getFigurePtr(figureId);
	...
	Vector3D spokeEnd, spokeDir, gradDir;
	double r;

	Vector3D hubPos = quadAtom->getX();
	...
	{
		std::cout << "Error: Unsupported spoke for normal adjustment" << std::flush << std::endl;
		exit(0);
	}

	spokeEnd = hubPos + r*spokeDir;

	gradDir = binaryDistanceMap->getGradDistance(spokeEnd);

	Vector3D diff = spokeEnd - hubPos;

	double num = gradDir * diff;
	double denom = gradDir * gradDir;
	double t = num / denom;

	if (spokeId == 0)
	{
		quadAtom->setU0(gradDir);
		quadAtom->setR(0,t);
	}
	else // only other choice is 1 b/c of earlier test
	{
		quadAtom->setU1(gradDir);
		quadAtom->setR(1,t);
	}
...
M3DSpoke* Match::interpolateSpoke(M3DFigure *figure, double u, double v, int side)
{
	...
	std::cout << "Call to Match::interpolateSpoke should NOT be happening" << std::endl;
...
{
	M3DInterpolater *interp = new M3DInterpolater(targetObject->getFigurePtr(figureId));
	...
	for (u = 0; u < umax; u++)
    {
		...
		for (v = 0; v < vmax; v++)
		{
			...
			for (i = 0; i < steps.size(); i++)
			{
				...
				{
					spokes.push_back(interp->interpolateSpoke(figure, u + steps[i], v + steps[j], 0));
					spokes.push_back(interp->interpolateSpoke(figure, u + steps[i], v + steps[j], 1));

					spokes.push_back(interp->interpolateSpoke(figure, u, v + steps[j], 0));
					spokes.push_back(interp->interpolateSpoke(figure, u, v + steps[j], 0));

					spokes.push_back(interp->interpolateSpoke(figure, u + steps[i], v, 1));
				}
            }
		}
    }
		...
		for(j = 0; j < steps.size(); j++)
        {
			if (u == 0)
            {
				if (v == 0)
				{
					...
					spokes.push_back(interp->interpolateSpoke(figure, u + steps[i], v, 0));
					spokes.push_back(interp->interpolateSpoke(figure, u + steps[i], v, 1));

					spokes.push_back(interp->interpolateSpoke(figure, u, v + steps[i], 0));
					spokes.push_back(interp->interpolateSpoke(figure, u, v + steps[i], 1));
				}
				else if (v == vmax)
				{
					spokes.push_back(interp->interpolateSpoke(figure, u + steps[i], v - steps[j], 0));
				...
				else
				{
					...
					spokes.push_back(interp->interpolateSpoke(figure, u, v + steps[j], 0));
					spokes.push_back(interp->interpolateSpoke(figure, u, v + steps[j], 1));
...
vector<M3DSpoke> Match::getRelevantSpokes(M3DObject* targetObject, int level, int figureId, int atomId, int spokeId)//spoke id is top or bottom
{
	...
	M3DSpoke thisSpoke = M3DSpoke(X, U, R);
        ...
        for (i = 0; i < steps.size(); i++)
        {
            for(j = 0; j < steps.size(); j++)
            {
                if (u == 0)
                {
                    if (v == 0)
                    {
                        spokes.push_back(interp->interpolateSpoke(figure, u + steps[i], v + steps[j], spokeId));
                        spokes.push_back(interp->interpolateSpoke(figure, u + steps[i], v, spokeId));
                        spokes.push_back(interp->interpolateSpoke(figure, u, v + steps[i], spokeId));
                    }
                    else if (v == vmax)
                    {
                        spokes.push_back(interp->interpolateSpoke(figure, u + steps[i], v - steps[j], spokeId));
                    ...
                    else
                    {
                        ...
                        spokes.push_back(interp->interpolateSpoke(figure, u + steps[i], v - steps[j], spokeId));
                        spokes.push_back(interp->interpolateSpoke(figure, u + steps[i], v, spokeId));
                        spokes.push_back(interp->interpolateSpoke(figure, u, v + steps[j], spokeId));
                        spokes.push_back(interp->interpolateSpoke(figure, u, v - steps[j], spokeId));
                    }
                }
                else if (u == umax)
...
void Match::GetCrestSpokes(vtkSmartPointer<vtkSRep> srepfig, int level, vector<M3DSpoke>& spokes, vtkIdType vtkAtomId, int spokeId){
