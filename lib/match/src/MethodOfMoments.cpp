...
void MethodOfMoments::figureMoments(M3DFigure * figure, double & volume, Vector3D & center, Quat & rotation)
{
	...
	*/
    ...
    int level = 3; // A high subdivision level allows crude approx.
    int nTiles;
	Bpoint * bPointList;
...
// Again, swiped from Tom's DamFit code
{
    ...
    if(ret != 0)
        ...
        double mydet = rotMat(0,0)*(rotMat(1,1)*rotMat(2,2)-rotMat(2,1)*rotMat(1,2)) -
                     rotMat(1,0)*(rotMat(0,1)*rotMat(2,2)-rotMat(2,1)*rotMat(0,2)) +
                     rotMat(2,0)*(rotMat(0,1)*rotMat(1,2)-rotMat(1,1)*rotMat(0,2));
