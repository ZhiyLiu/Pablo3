...
void P3DUserInterfaceCallback::readSelectedLinks()
{
    Fl_File_Chooser fc("\/playpen\/workspace\/proj_multiobject\/", "*.txt", Fl_File_Chooser::SINGLE, "Select file");
    fc.callback(NULL);
    ...
    //std::string str_name("\/playpen\/workspace\/demo_multiobject\/selected_links\.txt");
    userInterface->control->showSelectedLinks(filename);
}
...
void P3DUserInterfaceCallback::toggleFastInterp(int val) {
	std::cout << "toggle" << std::endl;
	//std::cout << val << std::endl;

	userInterface->modelView->toggleFastInterp(val);
	updateSurface();
...
void P3DUserInterfaceCallback::computeLinkingStructure()
{
    userInterface->control->computeLinkingStructure();
//    update();
}

void P3DUserInterfaceCallback::computeGeoEntropy()
{
    userInterface->control->computeGeoEntropy();
}
