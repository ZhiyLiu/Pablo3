...
void P3DView::setTilesColor(float *color)
{
...
void P3DView::useDefaultColor(bool yesno)
{
...
void P3DView::tileSetSolid(bool yesno)
{
...
//	cout << "depth = " << depth << endl;
	...
	for (nFigure = 0; nFigure < object->getFigureCount(); nFigure++) {
		...
		Pointlist_server2 * pList = landmarkSurface->getPList(nFigure);
		if (pList == NULL)
			continue;
...
void P3DView::updateImagePlanes(bool final)
{
       gMasterDispListIsValid = false;
       imagePlanesDone = false;
       imageStable = final;
...
void P3DView::resize(int x, int y, int w, int h)
{
       if (w > h)
           w = h;
       else if (h > w)
	...
	if (control->getPartialDrawingMode() == true)
	{
		...
		if (surfaceStyle != M3D_NONE) {
			if (! blendSurface ||
				...
				surfaceRenderer.setFastInterp(fastInterp);
				...
				if (! blendedRendererRunning) {
					...
					if (object->isModified() || blendingLevel != surfaceLevel) {
						...
						blendedSurfaceRenderer.setFastInterp(fastInterp);
...
/*  if(viewType == YZ_ORTHO)
    ...
    else
    {
        ...
        if (object != NULL)
        {
            ...
            if(object->initLinkingStructure)
            {
                object->renderLinkingStructure();
                object->initLinkingStructure = false;
            }
