...
int main(int argc, char *argv[])
{
    ...
    /*vtkSmartPointer<InterpolatedCortexError> error = vtkSmartPointer<InterpolatedCortexError>::New();
    error->SetOriginalSurface(pial);
    error->SetInterpolatedSurface(interpolatedmedialspokes);
    error->SetMapLabels(alllabelsmap[0]);
    error->SetVectLabels(alllabels[0]);
    error->Update();
    error->Print(cout);*/
    ...
    byuwriter->SetGeometryFileName("pialInterpolatedLevel3.byu");
    byuwriter->Write();

    byuwriter->SetInput(pial);
    byuwriter->SetGeometryFileName("pial.byu");
    byuwriter->Write();

    byuwriter->SetInput(orig);
    byuwriter->SetGeometryFileName("orig.byu");
    byuwriter->Write();

    /*M3DQuadFigure* cortex = NewQuadFigure(pointsIds.size(), pointsIds[0].size());


    for(unsigned i = 0; i < pointsIds.size(); i++){
         for(unsigned j = 0; j < pointsIds[i].size(); j++){

             double point[3];
             vtkIdType srepspokeid = pointsIds[i][j];
             srepfig->GetPoint(srepspokeid, point);

             M3DQuadPrimitive* prim = dynamic_cast<M3DQuadPrimitive*>(cortex->getPrimitivePtr(i, j));
