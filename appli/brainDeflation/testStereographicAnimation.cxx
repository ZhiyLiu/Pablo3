...
int main(int argc, char *argv[])
{
    ...
    vtkSmartPointer<vtkSRep> srepfig1 = vtkSmartPointer<vtkSRep>::New();
    vtkSmartPointer< vtkPoints > hubpos1 = vtkSmartPointer< vtkPoints >::New();
    vtkSmartPointer<vtkCellArray> cellarray1 = vtkSmartPointer<vtkCellArray>::New();

    vtkSRep::VectorSRepIdsType pointsIds1;

    vtkSRep::RadiusVectorType allradius1;
    vtkSRep::SpokesVectorType allspokes1;
    ...
    for(unsigned xn = 0; xn < steps.size(); xn++){
        x += steps[xn];
        double y = -logsum;
        pointsIds1.push_back(vtkSRep::VectorIdsType());
        for(unsigned yn = 0; yn < steps.size(); yn++){
            y += steps[yn];
            ...
            pointsIds1[(int)xn].push_back(hubpos1->InsertNextPoint(midpoint[0], midpoint[1], midpoint[2]));
            ...
            allspokes1.push_back(vnlspokes);
            allradius1.push_back(radius);
        }

    }


    for(unsigned i = 0; i < pointsIds1.size() - 1; i++){
         for(unsigned j = 0; j < pointsIds1[i].size() - 1; j++){

             vtkSmartPointer<vtkQuad> quad = vtkSmartPointer<vtkQuad>::New();
             quad->GetPointIds()->SetId(0, pointsIds1[i][j]);
             quad->GetPointIds()->SetId(1, pointsIds1[i+1][j]);
             quad->GetPointIds()->SetId(2, pointsIds1[i+1][j+1]);
             quad->GetPointIds()->SetId(3, pointsIds1[i][j+1]);

             //quad->Print(cout);

             cellarray1->InsertNextCell(quad);

         }
     }

    srepfig1->SetPoints(hubpos1);
    srepfig1->SetPolys(cellarray1);
    srepfig1->SetAllSpokes(allspokes1);
    srepfig1->SetAllSpokesRadius(allradius1);


    vtkSmartPointer<vtkSRep> srepfigmov = vtkSmartPointer<vtkSRep>::New();
    srepfigmov->SetPoints(hubpos1);
    srepfigmov->SetPolys(cellarray1);
    srepfigmov->SetAllSpokes(allspokes1);
    srepfigmov->SetAllSpokesRadius(allradius1);
    ...
    for(double t = 0; t <= 1.01; t+=0.01){
        ...
        vtkSmartPointer< vtkPoints > hubposmov = vtkSmartPointer< vtkPoints >::New();

        for(unsigned i = 0; i < hubpos1->GetNumberOfPoints(); i++){

            double p0[3];
            hubpos->GetPoint(i,p0);

            double p1[3];
            hubpos1->GetPoint(i,p1);

            double p2[3];
            p2[0] = (1-t)*p0[0] + (t)*p1[0];
            p2[1] = (1-t)*p0[1] + (t)*p1[1];
            p2[2] = (1-t)*p0[2] + (t)*p1[2];

            hubposmov->InsertNextPoint(p2);
        }

        srepfigmov->SetPoints(hubposmov);

        vtkSmartPointer< vtkSRepInterpolateMedialSheet > medialsheetinterpolator = vtkSmartPointer< vtkSRepInterpolateMedialSheet >::New();
        medialsheetinterpolator->SetInput(srepfigmov);
        if(t < 1.0){
            medialsheetinterpolator->SetInterpolationLevel(1);
        }else{
            medialsheetinterpolator->SetInterpolationLevel(3);
        }
        ...
        if(t < 1.0){
            ...
            medialsheetactor->GetProperty()->SetRepresentationToSurface();
        }
        ...
        medialspokesinterpolator->SetInput(srepfigmov);
        ...
        visuprimitives->SetInput(srepfigmov);
