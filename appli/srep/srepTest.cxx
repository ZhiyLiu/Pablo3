...
void help(char* execname){
    ...
    cout<<"-m <model name> ";
    cout<<"-s <surface data .vtk format> ";
...
int main(int argc, char *argv[])
{
    ...
    for(int i = 1; i < argc; i++){
        ...
        if(string(argv[i]) == "--h" || string(argv[i]) == "--help"){
         ...
         if(surfacefilename.compare("")!=0){
             vtkSmartPointer<vtkPolyDataReader> polyreader = vtkSmartPointer<vtkPolyDataReader>::New();
             polyreader->SetFileName(surfacefilename.c_str());
             polyreader->Update();
             polyreader->GetOutput();

             vtkSmartPointer<vtkPolyDataMapper> surfacemapper = vtkSmartPointer<vtkPolyDataMapper>::New();
             surfacemapper->SetInput(polyreader->GetOutput());
             vtkSmartPointer<vtkActor>  surfaceactor = vtkActor::New();
         ...
         medialspokesactor->GetProperty()->SetColor(1,0,0);
